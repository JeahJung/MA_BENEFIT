...
capture log close
log using "2.2.1 PHR-ED IP EventStudyInteract w plan FE 3-20-2026.log", replace
*************************************************************************************
* Purpose: Use EventStudyInteract, run DiD model for Any expanded PHR for nondual & dual separately after baseline matching.
*          1. Overall
...
foreach y in $outcome { 
    ...
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2 P_3, cohort(g_PHR) control_cohort(never_PHR) covariates($supp_benefit $X $condition) absorb(i.plan_fe i.year) vce(cluster plan_fe)
