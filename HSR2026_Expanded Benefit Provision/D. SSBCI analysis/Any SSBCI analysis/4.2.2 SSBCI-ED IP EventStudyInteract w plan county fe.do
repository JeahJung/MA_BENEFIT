...
capture log close
log using "3.2.2 SSBCI-ED IP EventStudyInteract w plan county fe 3-20-2025.log", replace
*************************************************************************************
* Purpose: Use EventStudyInteract, run DiD model for Any SSBCI for nondual & dual separately after baseline matching.
*          1. Overall
...
foreach y in $outcome {
    ...
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2, cohort(g_SSBCI) control_cohort(never_SSBCI) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
