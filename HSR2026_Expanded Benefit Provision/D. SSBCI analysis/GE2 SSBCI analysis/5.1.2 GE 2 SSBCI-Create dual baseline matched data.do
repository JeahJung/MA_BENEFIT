...
capture log close
log using "5.1.2 GE 2 SSBCI-Create dual baseline matched data 3-20-2026.log", replace
****************************************************************************************
...
* Output file: baseline_matched_SSBCI_dual_ge2_11match_enrollee_stayer
* 3-18-2026 by Ge
****************************************************************************************
    ...
    * must stay >=2 in all post years from cohort onward
    ...
    isid BENE_ID year
    save "enrollee_SSBCI_dual_ge2_11match_stayer.dta", replace
    count
...
clear
gen BENE_ID = .
save "FINAL_matched_sample_SSBCI_dual_ge2.dta", replace emptyok
save "FINAL_matched_bene_list_SSBCI_dual_ge2.dta", replace emptyok   // only IDs, for later merge

********************************************************************************
* 2020 ADOPTERS - baseline year = 2019
********************************************************************************
use "enrollee_SSBCI_dual_ge2_11match_stayer.dta", clear
keep if year == 2019
...
gen cohort = 2020
save "temp_2020_SSBCI_dual_ge2.dta", replace

* Append to master files
use "FINAL_matched_sample_SSBCI_dual_ge2.dta", clear
append using "temp_2020_SSBCI_dual_ge2.dta"
save "FINAL_matched_sample_SSBCI_dual_ge2.dta", replace

use "temp_2020_SSBCI_dual_ge2.dta", clear
keep BENE_ID
duplicates drop
append using "FINAL_matched_bene_list_SSBCI_dual_ge2.dta"
save "FINAL_matched_bene_list_SSBCI_dual_ge2.dta", replace
...
gen cohort = 2021
save "temp_2021_SSBCI_dual_ge2.dta", replace

use "FINAL_matched_sample_SSBCI_dual_ge2.dta", clear
append using "temp_2021_SSBCI_dual_ge2.dta"
save "FINAL_matched_sample_SSBCI_dual_ge2.dta", replace

use "temp_2021_SSBCI_dual_ge2.dta", clear
keep BENE_ID
duplicates drop
append using "FINAL_matched_bene_list_SSBCI_dual_ge2.dta"
save "FINAL_matched_bene_list_SSBCI_dual_ge2.dta", replace
...
gen cohort = 2022
save "temp_2022_SSBCI_dual_ge2.dta", replace

use "FINAL_matched_sample_SSBCI_dual_ge2.dta", clear
append using "temp_2022_SSBCI_dual_ge2.dta"
save "FINAL_matched_sample_SSBCI_dual_ge2.dta", replace

use "temp_2022_SSBCI_dual_ge2.dta", clear
keep BENE_ID
duplicates drop
append using "FINAL_matched_bene_list_SSBCI_dual_ge2.dta"
save "FINAL_matched_bene_list_SSBCI_dual_ge2.dta", replace
...
erase temp_2020_SSBCI_dual_ge2.dta
erase temp_2021_SSBCI_dual_ge2.dta
erase temp_2022_SSBCI_dual_ge2.dta
...
********************************************************************************
use "FINAL_matched_bene_list_SSBCI_dual_ge2.dta", clear
duplicates report BENE_ID
duplicates drop BENE_ID, force
save "FINAL_matched_bene_list_CLEAN_SSBCI_dual_ge2.dta", replace

use "enrollee_SSBCI_dual_ge2_11match_stayer.dta", clear

* Keep ONLY the matched beneficiaries (treated + their controls)
merge m:1 BENE_ID using "FINAL_matched_bene_list_CLEAN_SSBCI_dual_ge2.dta", keep(match) nogen
...
save "baseline_matched_SSBCI_dual_ge2_11match_enrollee_stayer.dta", replace
count
tab treated
