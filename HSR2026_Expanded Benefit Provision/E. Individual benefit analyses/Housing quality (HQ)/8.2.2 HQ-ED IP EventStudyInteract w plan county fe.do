...
capture log close
log using "8.2.2 HQ-ED IP EventStudyInteract w plan county fe 3-22-2026.log", replace
*************************************************************************************
* Purpose: Use EventStudyInteract, run DiD model for HQ for nondual & dual separately after baseline matching.
*          1. Overall
*          2. limit to frrailty
...
foreach y in $outcome {
    ...
    di "************ Matched Non-Dual eventstudyinteract - `y' of HQ ************"
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2, cohort(g_HQ) control_cohort(never_HQ) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
...
di "************ Matched Dual eventstudyinteract - `y' of HQ ************"
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2, cohort(g_HQ) control_cohort(never_HQ) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
...
foreach y in $outcome {
    ...
    di "************ Matched Non-Dual limit to frailty eventstudyinteract - `y' of HQ ************"
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2, cohort(g_HQ) control_cohort(never_HQ) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
...
di "************ Matched Dual limit to frailty eventstudyinteract - `y' of HQ ************"
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2, cohort(g_HQ) control_cohort(never_HQ) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
