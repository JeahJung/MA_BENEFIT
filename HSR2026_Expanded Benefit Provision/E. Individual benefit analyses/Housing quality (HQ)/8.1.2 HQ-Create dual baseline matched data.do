...
capture log close
log using "8.1.2 HQ-Create dual baseline matched data 3-22-2026.log", replace
****************************************************************************************
...
* Output file: baseline_matched_HQ_dual_11match_enrollee_stayer
* 3-22-2026 by Ge
****************************************************************************************
    ...
    isid BENE_ID year
    save "enrollee_HQ_dual_11match_stayer.dta", replace
    count
...
clear
gen BENE_ID = .
save "FINAL_matched_sample_HQ_dual.dta", replace emptyok
save "FINAL_matched_bene_list_HQ_dual.dta", replace emptyok   // only IDs, for later merge

********************************************************************************
* 2020 ADOPTERS - baseline year = 2019
********************************************************************************
use "enrollee_HQ_dual_11match_stayer.dta", clear
keep if year == 2019
...
gen cohort = 2020
save "temp_2020_HQ_dual.dta", replace

* Append to master files
use "FINAL_matched_sample_HQ_dual.dta", clear
append using "temp_2020_HQ_dual.dta"
save "FINAL_matched_sample_HQ_dual.dta", replace

use "temp_2020_HQ_dual.dta", clear
keep BENE_ID
duplicates drop
append using "FINAL_matched_bene_list_HQ_dual.dta"
save "FINAL_matched_bene_list_HQ_dual.dta", replace
...
gen cohort = 2021
save "temp_2021_HQ_dual.dta", replace

use "FINAL_matched_sample_HQ_dual.dta", clear
append using "temp_2021_HQ_dual.dta"
save "FINAL_matched_sample_HQ_dual.dta", replace

use "temp_2021_HQ_dual.dta", clear
keep BENE_ID
duplicates drop
append using "FINAL_matched_bene_list_HQ_dual.dta"
save "FINAL_matched_bene_list_HQ_dual.dta", replace
...
gen cohort = 2022
save "temp_2022_HQ_dual.dta", replace

use "FINAL_matched_sample_HQ_dual.dta", clear
append using "temp_2022_HQ_dual.dta"
save "FINAL_matched_sample_HQ_dual.dta", replace

use "temp_2022_HQ_dual.dta", clear
keep BENE_ID
duplicates drop
append using "FINAL_matched_bene_list_HQ_dual.dta"
save "FINAL_matched_bene_list_HQ_dual.dta", replace
...
erase temp_2020_HQ_dual.dta
erase temp_2021_HQ_dual.dta
erase temp_2022_HQ_dual.dta
...
********************************************************************************
use "FINAL_matched_bene_list_HQ_dual.dta", clear
duplicates report BENE_ID
duplicates drop BENE_ID, force
save "FINAL_matched_bene_list_CLEAN_HQ_dual.dta", replace

use "enrollee_HQ_dual_11match_stayer.dta", clear

* Keep ONLY the matched beneficiaries (treated + their controls)
merge m:1 BENE_ID using "FINAL_matched_bene_list_CLEAN_HQ_dual.dta", keep(match) nogen
...
save "baseline_matched_HQ_dual_11match_enrollee_stayer.dta", replace
count
tab treated
