...
capture log close
log using "6.2.2 IHSS-ED IP EventStudyInteract w plan county fe 3-22-2026.log", replace
*************************************************************************************
* Purpose: Use EventStudyInteract, run DiD model for IHSS for nondual & dual separately after baseline matching.
*          1. Overall
...
foreach y in $outcome {
    ...
    di "************ Matched Non-Dual  eventstudyinteract - `y' of IHSS ************"
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2 P_3, cohort(g_IHSS) control_cohort(never_IHSS) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
...
di "************ Matched Dual eventstudyinteract - `y' of IHSS ************"
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2 P_3, cohort(g_IHSS) control_cohort(never_IHSS) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
...
foreach y in $outcome {
    ...
    di "************ Matched Non-Dual limit to frailty eventstudyinteract - `y' of IHSS ************"
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2 P_3, cohort(g_IHSS) control_cohort(never_IHSS) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
...
di "************ Matched Dual limit to frailty eventstudyinteract - `y' of IHSS ************"
    eventstudyinteract `y'_ind M_5 M_4 M_3 M_2 P_0 P_1 P_2 P_3, cohort(g_IHSS) control_cohort(never_IHSS) covariates($supp_benefit $X $condition) absorb(i.plan_county i.year) vce(cluster plan_county)
